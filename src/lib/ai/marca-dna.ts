...
export interface MarcaDNAResult {
  id?: string;
  empresa_id: string;
...
export function parseGPTJson(raw: string, fallback: any): any {
...
{"resumo_visual":"descrição do estilo visual","tom_legendas":"tom das legendas","temas_recorrentes":["tema1","tema2","tema3"],"estilo_visual":"padrão visual dominante","formatos_mais_usados":["formato1","formato2"],"hashtags_frequentes":["#tag1","#tag2"],"frequencia_postagem":"estimativa","engajamento_medio":"estimativa percentual","pontos_fortes":["ponto1","ponto2"],"pontos_fracos":["ponto1","ponto2"]}`,
  ...
  return parseGPTJson(raw, {
    username,
    error: "Failed to parse IG analysis",
  });
}
...
export async function analyzeSite(
  ...
  try {
    ...
    if (!siteResponse.ok) {
      console.warn(
        `[MarcaDNA] Site retornou status ${siteResponse.status}: ${url}`
      );
      return null;
    }
...
export async function synthesizeDNA(
  empresaNome: string,
  empresaNicho: string,
...
export function getMockMarcaDNA(empresaId: string): MarcaDNAResult {
  return {
