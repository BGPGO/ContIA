...
import { rejectPost } from "@/lib/approvals";

// POST /api/post-approvals/[id]/reject — Rejeita um post
...
export async function POST(
  ...
  try {
    ...
    // Body — comment é OBRIGATÓRIO para rejeição
    let body: { comment?: string } = {};
    try {
      body = await request.json();
    } catch {
      // noop — validamos abaixo
    }
    ...
    if (!empresa) {
      return NextResponse.json(
        { error: "Sem permissão para rejeitar este post." },
        { status: 403 }
      );
    }

    const result = await rejectPost(supabase, {
    ...
    console.error("[post-approvals/id/reject] POST error:", errorMsg);

    if (errorMsg.includes("não encontrad")) {
